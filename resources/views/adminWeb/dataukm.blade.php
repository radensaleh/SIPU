  ...
  <div class="container-fluid">
    <div class="navbar-collapse collapse in">
      <ul class="nav navbar-nav navbar-left">
        <li class="navbar-title">Data UKM</li>
      </ul>
...
<div class="btn-floating" id="help-actions">
  ...
  <div class="toggle-content">
    <ul class="actions">
      <li><a id="addDataUkm" data-toggle="modal" data-target="#addData"><span class="fa fa-plus-circle"></span> Add Data</a></li>
    </ul>
  </div>
</div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><span class="fa fa-user-plus"></span> Add Data UKM</h4>
      </div>
      <div class="modal-body">
        <form id="modal-form-add" action="{{ route('data-ukm.store') }}" method="post" role="form">
          {{ csrf_field() }}
          <div class="form-group has-success">
            <label for="id_ukm" class="form-control-label">ID UKM</label>
            <input type="text" id="inputIdUkm" name="id_ukm" class="form-control" required/>
            <span class="text-warning" ></span>
          </div>
          <div class="form-group has-success">
            <label for="nama_ukm" class="form-control-label">Nama UKM</label>
            <input type="text" id="inputNamaUkm" name="nama_ukm" class="form-control" required />
          </div>
      </div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form id="modal-form-delete" method="post" action="{{ route('data-ukm.destroy', 'destroy') }}">
            {{ method_field('delete') }}
            {{ csrf_field() }}
      <div class="modal-body">
            <input type="hidden" name="id_ukm" id="cat_id" value="">
...
<div class="row">
  ...
  <tbody>
      @foreach($UKM as $key => $data)
        <tr>
          <td>{{ ++$key }}</td>
          <td>{{ $data->id_ukm }}</td>
          <td>{{ $data->nama_ukm }}</td>
          <td>
            <button class="btn btn-xs btn-warning" data-toggle="modal" data-target="#editData" data-id_ukm = "{{ $data->id_ukm }}" data-nama_ukm="{{ $data->nama_ukm }}">
                  <span class="fa fa-edit"></span> Edit
            </button>
            <button class="btn btn-xs btn-danger" data-toggle="modal" data-target="#deleteData" data-id_ukm = "{{ $data->id_ukm }}">
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <form id="modal-form-edit" method="post" action="{{ route('data-ukm.update', 'update') }}">
              {{ method_field('patch') }}
              {{ csrf_field() }}
        <div class="modal-body">
              <input type="hidden" name="id_ukm" id="cat_id" value="">
            <div class="form-group has-warning">
              <label for="id_ukm" class="form-control-label">ID UKM</label>
              <input type="text" id="id_ukm" name="id_ukm" class="form-control"  required />
            </div>
            <div class="form-group has-warning">
              <label for="nama_ukm" class="form-control-label">Nama UKM</label>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
          ...
          $('#editData').on('show.bs.modal', function (event) {
            ...
            modal.find('.modal-body #nama_ukm').val(nama_ukm)
            modal.find('.modal-body #cat_id').val(id_ukm)
            $("#id_ukm").prop('disabled', true);
          });
                    ...
                    if( res.error == 0 ){
                      ...
                      swal(
                        ...
                        res.message,
                          ...
                          if(OK){
                            window.location.href = "{{ route('data-ukm') }}";
                          }
                        });
                    } else{
                        $('#addData').modal('hide');
